...
// Alle badge-PNG's lokaal, relatief t.o.v. de repo-root (badges/<slug>/<file>.png).
function pngFiles(dir, rel = 'badges') {
  const out = [];
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    const p = path.join(dir, e.name);
    const r = rel + '/' + e.name;
    if (e.isDirectory()) out.push(...pngFiles(p, r));
...
(async () => {
  const lokaal = pngFiles(BADGES);
  if (!lokaal.length) {
    console.error('⚠  Geen badge-PNG\'s in badges/ — niets te pushen (draai eerst de generator).');
    process.exit(0);
  }
  ...
  try {
    ...
    // Enkel de gewijzigde/toegevoegde PNG's purgen (verwijderde hoeven niet).
    const teZuiveren = staged.filter(f => f.endsWith('.png') && fs.existsSync(path.join(ROOT, f)));
    let ok = 0;
