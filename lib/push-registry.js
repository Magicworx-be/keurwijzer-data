...
try {
  ...
  const purgeUrl = 'https://purge.jsdelivr.net/gh/' + REPO + '/registry.json';
  try {
    ...
    https.get(purgeUrl, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200) console.log('  ✓ jsDelivr-cache gepurged');
        else console.log('  ⚠ jsDelivr-purge status ' + res.statusCode + ' (niet kritiek)');
      });
    }).on('error', () => console.log('  ⚠ jsDelivr-purge mislukt (niet kritiek)'));
  } catch { /* negeren */ }
} catch (err) {
